CREATE DEFINER=`root`@`localhost` PROCEDURE `data_validation`()
-- Apply validation rules - failures are recorded as erros
BEGIN
...
INSERT INTO adt_trx_proc_state(file_id, trx_id, proc_id, seq_id, proc_type, trx_proc_state, user_id, message)
  SELECT file_id, trx_id, 1201, 0, 'L', 'E', 'admin01', '{S17:Invalid marketers code 2}'  
  FROM dat_transaction_data LEFT JOIN dat_agent_profile
...
INSERT INTO adt_trx_proc_state(file_id, trx_id, proc_id, seq_id, proc_type, trx_proc_state, user_id, message)
  SELECT file_id, trx_id, 1201, 0, 'L', 'E', 'admin01', '{S17:Invalid marketers code 4}'  
  FROM dat_transaction_data LEFT JOIN dat_agent_profile
...
INSERT INTO adt_trx_proc_state(file_id, trx_id, proc_id, seq_id, proc_type, trx_proc_state, user_id, message)
  SELECT currtrx.file_id, currtrx.trx_id, 1201, 0, 'L', 'W', 'admin01', '{S18:Warning - marketers code 3 history mismatch}'  
  FROM dat_transaction_data AS currtrx LEFT JOIN dat_transaction_data AS history
	ON currtrx.policy_no = history.policy_no 
		AND CAST(currtrx.period AS UNSIGNED INTEGER) = CAST(history.period AS UNSIGNED INTEGER) + 1  
  WHERE currtrx.file_id = 29 
...
INSERT INTO adt_trx_proc_state(file_id, trx_id, proc_id, seq_id, proc_type, trx_proc_state, user_id, message)
  SELECT currtrx.file_id, currtrx.trx_id, 1201, 0, 'L', 'W', 'admin01', '{S19:Warning - marketers code 4 history mismatch}'  
  FROM dat_transaction_data AS currtrx LEFT JOIN dat_transaction_data AS history
	ON currtrx.policy_no = history.policy_no 
		AND CAST(currtrx.period AS UNSIGNED INTEGER) = CAST(history.period AS UNSIGNED INTEGER) + 1  
  WHERE currtrx.file_id = 29 
